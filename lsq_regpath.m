function [rho_path, beta_path] = lsq_regpath(X,y,wt,D,pentype,penparam)
% LSQ_REGPATH Calculate the solution path of  
%   argmin .5*sum(wt.*(y-X*beta).^2)+sum(penfun(D*beta))
%
% INPUT:
...
[rho_path,beta_path] = ...
    lsq_sparsepath(X*T,y,wt,penidx,maxpreds,pentype,penparam);

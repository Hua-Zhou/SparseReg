...
!
      ...
      END IF
!
!     CHECK FOR PROPER INPUT ARGUMENT DIMENSIONS
!
      N = MXGETM(PRHS(2))
      P = MXGETN(PRHS(2))
      IF (MXGETM(PRHS(1))/=P.AND.MXGETN(PRHS(1))/=P) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:Input1Size','Input 1 has incompatible size.')
      ELSEIF (MXGETM(PRHS(3))/=N.AND.MXGETN(PRHS(3))/=N) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:Input3Size','Input 3 has incompatible size.')
      ELSEIF (MXGETM(PRHS(4))/=N.AND.MXGETN(PRHS(4))/=N) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:Input4Size','Input 4 has incompatible size.')
      ELSEIF (MXGETM(PRHS(6))/=P.AND.MXGETN(PRHS(6))/=P) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:Input6Size','Input 6 has incompatible size.')
      ELSEIF (MXGETM(PRHS(7))/=P.AND.MXGETN(PRHS(7))/=P) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:Input3Size','Input 7 has incompatible size.')
      END IF
!
!     CHECK FOR PENALTY TYPE WITH PROPER PENALTY INDEX PARAMETER
!
      ...
      IF (TRIM(PENTYPE)=="ENET".AND.(PENPARAM(1)<1.OR.PENPARAM(2)>2)) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:ENETParam','Parameter for ENET penalty should be in [1,2].')
      ELSEIF (TRIM(PENTYPE)=="LOG".AND.PENPARAM(1)<ZERO) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:LOGParam','Parameter for LOG penalty should be nonnegative.')
      ELSEIF (TRIM(PENTYPE)=="MCP".AND.PENPARAM(1)<=ZERO) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:MCPParam','Parameter for MCP penalty should be positive.')
      ELSEIF (TRIM(PENTYPE)=="POWER".AND.(PENPARAM(1)<=ZERO.OR.PENPARAM(1)>2)) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:POWERParam','Parameter for POWER penalty should be in (1,2].')
      ELSEIF (TRIM(PENTYPE)=="SCAD".AND.PENPARAM(1)<=TWO) THEN
         CALL MEXERRMSGIDANDTXT('MATLAB:lsqsparse:SCADParam','Parameter for SCAD penalty should greater than 2.')
      END IF
!

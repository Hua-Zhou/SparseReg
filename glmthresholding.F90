...
!		
      ...
      REAL(KIND=DBLE_PREC), ALLOCATABLE, DIMENSION(:) :: PENPARAM,WT,XMIN,Y
      REAL(KIND=DBLE_PREC), ALLOCATABLE, DIMENSION(:,:) :: X,C
!
...
!
      ...
      P = MXGETN(PRHS(1)) 
      SIZE = N*P
      ALLOCATE(C(N,P),WT(N),X(N,P),XMIN(P),Y(N))
      CALL MXCOPYPTRTOREAL8(MXGETPR(PRHS(1)),X,SIZE)
      CALL MXCOPYPTRTOREAL8(MXGETPR(PRHS(2)),C,SIZE)
...
!
      DO J=1,P
         XMIN(J) = GLM_THRESHOLDING(X(:,J),C(:,J),Y,WT,LAMBDA,PENPARAM(1),PENTYPE,MODEL)
      END DO
